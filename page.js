...
import { Shield, Eye, Users, Play, BookOpen, Search, UserPlus, X, HelpCircle, ArrowRight } from 'lucide-react';
...
export default function V0MafiaApp() {
  const [currentScreen, setCurrentScreen] = useState('home'); // home, lobby, playing, rules, discover
    ...
    <div style={{ backgroundColor: '#121212', color: '#ffffff', minHeight: '100vh', fontFamily: 'system-ui, sans-serif', paddingBottom: '70px' }}>
      ...
      <header style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #222', backgroundColor: '#1a1a1a' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontWeight: 'bold', fontSize: '18px', letterSpacing: '1px' }}>مافيا الحارة</span>
        </div>
      ...
      <main style={{ maxWidth: '480px', margin: '0 auto', padding: '20px' }}>
        ...
        {currentScreen === 'home' && (
          <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '20px', marginTop: '20px' }}>
            <div style={{ background: 'linear-gradient(180deg, #1f1a1a 0%, #1a1a1a 100%)', border: '1px solid #331d1d', borderRadius: '24px', padding: '30px 20px' }}>
              <div style={{ width: '70px', height: '70px', backgroundColor: '#e53e3e20', color: '#e53e3e', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyCenter: 'center', margin: '0 auto 15px auto', border: '1px solid #e53e3e40' }}>
                <Shield size={36} />
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <button
                ...
                style={{ padding: '16px', borderRadius: '14px', border: 'none', backgroundColor: '#e53e3e', color: '#fff', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', shadow: '0 4px 12px rgba(229,62,62,0.3)' }}
              >
        ...
        {currentScreen === 'lobby' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ backgroundColor: '#1a1a1a', border: '1px solid #333', borderRadius: '20px', padding: '20px', textAlign: 'center' }}>
              ...
              <p style={{ color: '#aaa', fontSize: '13px' }}>شارك هذا الرمز مع أصدقائك للانضمام</p>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '200px', overflowY: 'auto' }}>
                {players.map((p) => (
                  <div key={p.id} style={{ padding: '12px 16px', backgroundColor: '#242424', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #333' }}>
                    <span>{p.name}</span>
                    {p.isHost && <span style={{ fontSize: '10px', backgroundColor: '#e53e3e20', color: '#e53e3e', padding: '2px 8px', borderRadius: '4px', border: '1px solid #e53e3e40' }}>المضيف</span>}
                  </div>
                ))}
              </div>
            </div>
        ...
        {currentScreen === 'rules' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <h2 style={{ fontSize: '22px', fontWeight: 'bold', color: '#e53e3e' }}>قواعد ودليل اللعبة</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {ROLES.map((r) => (
                <div key={r.id} style={{ backgroundColor: '#1a1a1a', border: '1px solid #222', borderRadius: '16px', padding: '16px', display: 'flex', gap: '14px', alignItems: 'center' }}>
                  ...
                  <div>
                    <h4 style={{ fontWeight: 'bold', color: '#fff' }}>{r.name} <span style={{ fontSize: '11px', color: '#e53e3e', fontWeight: 'normal' }}>({r.team})</span></h4>
                    <p style={{ fontSize: '12px', color: '#888', marginTop: '2px' }}>{r.desc}</p>
                  </div>
                </div>
              ))}
            </div>
            <button onClick={() => setCurrentScreen('home')} style={{ padding: '14px', backgroundColor: '#222', border: 'none', color: '#fff', borderRadius: '12px', marginTop: '10px', cursor: 'pointer' }}>
              العودة للرئيسية
            </button>
          </div>
        )}
      ...
      {/* --- JOIN MODAL --- */}
      {showJoinModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyCenter: 'center', padding: '20px', zIndex: 100 }}>
          <div style={{ backgroundColor: '#1a1a1a', border: '1px solid #333', borderRadius: '24px', padding: '24px', width: '100%', maxWidth: '360px', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <input
                type="text"
                placeholder="رمز الغرفة (Code)"
                value={roomCode}
                onChange={(e) => setRoomCode(e.target.value)}
                style={{ padding: '12px', borderRadius: '10px', border: '1px solid #333', backgroundColor: '#242424', color: '#fff', textAlign: 'center', fontSize: '18px', letterSpacing: '2px' }}
              />
